...
```
mysql> SELECT 公司名称 FROM 通用认证信息 WHERE 证书状态=有效;
...
mysql> SELECT 公司名称 FROM 通用认证信息 WHERE id >= 2;
# id是int类型的不用加''
+--------------+
| 公司名称     |
+--------------+
...
```
mysql> SELECT id,公司名称 FROM 通用认证信息 WHERE id <> 2;
...
```
mysql> SELECT id,公司名称 FROM 通用认证信息 WHERE id BETWEEN 1 AND 2;
...
```
mysql> SELECT id,公司名称 FROM 通用认证信息 WHERE 认证项目 IS NULL;
...
```
mysql> SELECT id,公司名称,证书状态 FROM 通用认证信息 WHERE id NOT IN (1,3);
...
```
mysql> SELECT id,公司名称,证书状态 FROM 通用认证信息 WHERE id <> 2 AND 证书状态 = '有效';
+----+--------------+--------------+
| id | 公司名称     | 证书状态     |
+----+--------------+--------------+
|  1 | 小白公司     | 有效         |
...
```
mysql> SELECT id,公司名称,证书状态 FROM 通用认证信息 WHERE id <> 2 OR 证书状态 = '有效';
+----+--------------+--------------+
| id | 公司名称     | 证书状态     |
+----+--------------+--------------+
|  1 | 小白公司     | 有效         |
...
mysql> SELECT id,公司名称,认证项目,证书状态 FROM 通用认证信息 WHERE id != 2 OR 证书状态='过期' AND 认证项目='电气';
